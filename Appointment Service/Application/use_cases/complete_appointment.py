...
class CompleteAppointmentUseCase:
    ...
    async def execute(self, appointment_id: UUID7, doctor_id: UUID7) -> AppointmentResponse:
        ...
        if appointment.doctor_id != doctor_id:
            raise UnauthorizedActionError("Only assigned doctor can complete this appointment")

        if not appointment.can_transition_to(AppointmentStatus.COMPLETED):
            raise InvalidStatusTransitionError("Appointment cannot be marked completed from current status")
