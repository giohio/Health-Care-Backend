...
if [ -d /app/alembic ]; then
  ...
  if ! upgrade_head >/tmp/alembic_upgrade.log 2>&1; then
    if grep -q "Can't locate revision identified by" /tmp/alembic_upgrade.log; then
      ...
      echo "Detected NOT NULL constraint violation. Resetting appointments table and retrying..."
      psql "$PSQL_DATABASE_URL" -v ON_ERROR_STOP=1 -c "TRUNCATE TABLE appointments RESTART IDENTITY CASCADE;"
      if ! upgrade_head; then
