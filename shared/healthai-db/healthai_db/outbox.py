...
from healthai_db.base import Base, TimestampMixin, UUIDMixin
...
class OutboxEvent(Base, UUIDMixin, TimestampMixin):
    ...
    aggregate_id: Mapped[UUID7] = mapped_column(PG_UUID(as_uuid=True), nullable=False, index=True)
...
class OutboxWriter:
    ...
    @staticmethod
    async def write(session, aggregate_id: Any, aggregate_type: str, event_type: str, payload: dict) -> OutboxEvent:
        """Write single outbox event."""
        event = OutboxEvent(
            aggregate_id=aggregate_id, aggregate_type=aggregate_type, event_type=event_type, payload=payload
        )
        session.add(event)
        return event

    @staticmethod
    async def write_many(session, events: list[dict]) -> list[OutboxEvent]:
        """
