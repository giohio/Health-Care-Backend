...
from healthai_db import OutboxWriter, OutboxEvent, Base, create_session_factory
...
async def test_outbox_write_single(session_factory):
    ...
    async with session_factory() as session:
        ...
        result = await session.execute(
            select(OutboxEvent)
            .where(OutboxEvent.aggregate_id == appt_id)
        )
...
async def test_outbox_write_many(session_factory):
    ...
    async with session_factory() as session:
        ...
        result = await session.execute(
            select(OutboxEvent)
            .where(OutboxEvent.aggregate_type == 'user_events')
        )
