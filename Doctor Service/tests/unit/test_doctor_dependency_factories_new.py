...
def test_use_case_factories():
    repo = MagicMock()
    assert isinstance(get_list_specialties_use_case(repo), ListSpecialtiesUseCase)
...
def test_complex_use_case_factories():
    ...
    uc = get_enhanced_schedule_use_case(availability_repo, day_off_repo, service_offering_repo)
...
def test_submit_rating_use_case_factory():
    ...
    uc = get_submit_rating_use_case(rating_repo, doctor_repo, appointment_repo)
